          ...
          <div className="space-y-12">
            {EDUCATION.map((edu, index) => (
              ...
              >
                ...
                <div
                  className={`absolute left-8 md:left-1/2 transform -translate-x-1/2 flex items-center justify-center w-10 h-10 rounded-full border-4 z-10 ${theme.cardBg} ${
                    theme.id === 'cyber' ? 'border-green-500 shadow-neon' : theme.id === 'brutalist' ? 'border-black bg-white' : 'border-gray-200 shadow-sm'
                  }`}
                >
                ...
                <div className="ml-20 md:ml-0 w-full md:w-5/12">
                  <div className={`p-6 rounded-xl ${theme.cardBg} ${theme.cardBorder} hover:shadow-lg transition-shadow`}>
                    <span className={`inline-block px-2 py-1 mb-2 text-xs font-bold uppercase rounded ${theme.accent}`}>{edu.status}</span>
                    ...
                    {edu.details && <p className={`text-xs mt-2 pt-2 border-t border-gray-500/10 ${theme.textSecondary}`}>{edu.details}</p>}
                  </div>
                </div>
              </motion.div>
            ))}
