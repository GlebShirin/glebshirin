...
export const Projects: React.FC<ProjectsProps> = ({ theme }) => {
  ...
  const getIcon = (id: string, className = "w-8 h-8") => {
    switch(id) {
      case 'vala': return <Box className={className} />;
      case 'sevenoir': return <Layers className={className} />;
      case 'nvg': return <Zap className={className} />;
      case 'skilloton': return <Globe className={className} />;
      case '13players': return <Activity className={className} />;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between">
           <div>
             ...
             <div className={`h-1 w-20 ${theme.id === 'cyber' ? 'bg-green-500' : theme.id === 'brutalist' ? 'bg-black' : 'bg-blue-500/50'}`}></div>
           </div>
           <p className={`${theme.textSecondary} mt-4 md:mt-0 max-w-md text-right`}>
             A portfolio of active ecosystems, prototypes, and stealth R&D.
           </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {PROJECTS.map((project, index) => (
            ...
            >
              {/* Oracolure Mystery Effect */}
              {project.isSecret && (
                <div className={`absolute inset-0 z-20 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center transition-opacity duration-500 ${theme.id === 'neoglass' ? 'bg-black/40' : 'bg-black/80'}`}>
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                <div className={`p-3 rounded-lg ${theme.accent} bg-opacity-20`}>
                  {getIcon(project.id)}
                </div>
                {!project.isSecret && (
                  <ArrowUpRight className={`w-6 h-6 opacity-0 group-hover:opacity-100 transition-opacity ${theme.textPrimary}`} />
                )}
              </div>

              {/* Content */}
              <div className={`flex-grow relative z-10 ${project.isSecret ? 'blur-sm opacity-50' : ''}`}>
                <h3 className={`text-2xl font-bold mb-2 ${theme.fontHeading} ${theme.textPrimary}`}>
                  {project.title}
                </h3>
                <h4 className={`text-sm font-medium uppercase tracking-wider mb-4 ${theme.textSecondary}`}>
                  {project.subtitle}
                </h4>
                <p className={`mb-6 text-sm leading-relaxed ${theme.textSecondary}`}>
                  {project.description}
                </p>
              </div>
              ...
              <div className={`pt-6 mt-auto border-t border-gray-500/10 relative z-10 ${project.isSecret ? 'blur-sm opacity-50' : ''}`}>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs font-semibold uppercase ${theme.textPrimary}`}>
                    {project.status}
                  </span>
                </div>
              </div>
        ...
        {selectedProject && (
          ...
          >
            ...
            >
              ...
              <button 
                onClick={closeModal}
                className={`absolute top-4 right-4 p-2 rounded-full hover:bg-white/10 ${theme.textPrimary} transition-colors z-20`}
              >
                <X size={24} />
              </button>

              <div className="p-8">
                {/* Switch between Project Details and Contact Info */}
                {!showContactModal ? (
                  <>
                    {/* Modal Header */}
                    <div className="flex items-center gap-4 mb-6">
                      <div className={`p-4 rounded-xl ${theme.accent} bg-opacity-20`}>
                        {getIcon(selectedProject.id, "w-10 h-10")}
                      </div>
                      <div>
                        <h3 className={`text-3xl font-bold ${theme.fontHeading} ${theme.textPrimary}`}>
                          {selectedProject.title}
                        </h3>
                        <p className={`text-sm font-medium uppercase tracking-wider ${theme.textSecondary}`}>
                    ...
                    {/* Vision Section */}
                    <div className="mb-8">
                      <h4 className={`text-sm font-bold uppercase tracking-wider ${theme.textSecondary} mb-2`}>
                        Vision
                      </h4>
                      <p className={`text-lg italic font-light ${theme.textPrimary}`}>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-8 border-t border-white/10">
                      <button
                        ...
                        className={`flex-1 px-6 py-3 ${theme.buttonStyles} flex items-center justify-center gap-2 text-center cursor-pointer`}
                      >
                      ...
                      <div className="flex gap-2">
                         <button 
                           onClick={handlePitchDeckRequest}
                           className={`p-3 ${theme.buttonStyles} flex items-center justify-center rounded-lg`}
                           title="Contact"
                         >
                           <Send size={20} />
                         </button>
                  ...
                  /* Contact Modal Content - Compact Action Block */
                  <div className="flex flex-col items-center justify-center py-2">
                    <h3 className={`text-2xl font-bold ${theme.fontHeading} ${theme.textPrimary} mb-6`}>
                      Pitch Deck Request
                    </h3>
